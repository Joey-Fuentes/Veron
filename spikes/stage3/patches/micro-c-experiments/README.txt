...
REGENERATED AGAINST THE PIN -- READ THIS FIRST IF YOU SAW CI FAIL.

Every patch in this directory and in ../m2-planet/ was previously generated by
diffing against spikes/reference/m2-planet, which is an OLDER M2-Planet than
the pin the build actually uses (bd2fe4b). The two differ by dozens of lines
per file. The local verification loop applied the series to the vendored tree
and reported success; CI applied it to the pin and failed from 0003 onward.

The check said "applies clean from a fresh tree" the whole time. It was a fresh
copy of the WRONG tree.

A second fault compounded it: the generator used a hand-written list of source
files and omitted cc_emit.h, so the series applied and then failed to COMPILE.
It now discovers every differing file instead of listing them.

Both are the same mistake in different clothes -- a verification that checked
something adjacent to what CI does, rather than what CI does.
================================================================
