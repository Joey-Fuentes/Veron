/* A struct pointer in an array, reached ONLY through &arr[i].
 *
...
int main(void)
{
    ...
    i = 0;
    while (i < 8) { arr[i] = 0; i = i + 1; }

    /* the array is populated WITHOUT address-of, which 39 proves works, so
     * a failure here can only be the address-of path */
    arr[5] = &target;
