...
## Source conventions, and why they are not style preferences

`.github/workflows/stage0-selfhost.yml` compares this file against the
disassembly of the binary built from it, as a plain `diff`. That only works if
the source is written the way the disassembler prints. Each rule below was
derived by measuring, not chosen:

| rule | why |
|---|---|
| immediates as **lowercase minimal hex** — `#0xa`, not `#0x0A` or `#10` | GNU objdump prints every immediate that way, unconditionally. llvm-objdump does the same by default. |
| character literals as their hex value, character in a trailing comment — `cmp w0, #0x23  // '#'` | the binary holds 0x23; nothing records that the source wrote a character. The comment carries the readability, the code carries the exactness. |
| shift amounts stay **decimal** — `lsl #16` | both decoders print shift amounts decimal even beside a hex immediate. Converting them would *create* a difference. |
| no `movz Rd, #imm, lsl #s` — write `mov Rd, #<folded>` | the decoder folds the shift into the immediate and prints `mov`, so a `movz` line can never read back as written. Folding it in the source keeps one instruction and the same encoding. |
| **`.text` holds only code** — strings live in `.rodata` | a disassembler decodes data in `.text` as instructions, producing phantom labels and a padding word, and stopping the comparison from being total. |
| every referenced symbol has a **unique address** — see `bss_origin` | `__bss_start` is generated by the linker at the start of `.bss`, so whatever comes first there shares an address with it and the two decoders disagree about which name to print. |
| **memory offsets stay decimal** — `[sp, #16]`, not `[sp, #0x10]` | same rule as shift amounts: both decoders print addressing offsets decimal even beside a hex immediate. |
| `.equ` **values are hex too** | the value is inlined into the instruction, so `.equ HDR_LEN, 120` produces `mov x24, #0x78` in the disassembly and `#120` in the source. |
| negative immediates as the **64-bit unsigned value** — `#0xffffffffffffff9c`, not `#-100` or `#-0x64` | that is what the decoder prints for a MOVN. |

These last three were found on `elf` rather than `stage0-as`, which had no
memory offsets, no `.equ` reaching an instruction, and no negative literals.
The list is what has been measured, not what has been imagined — expect it to
grow the first time a new construct appears.

`ROUNDTRIP_BUDGET` in that workflow is a ratchet: the number of instructions
that fail to read back as written. It is currently `0` and may only fall.
Breaking any rule above will fail the step.

### Measured limit

30 of 785 lines cannot satisfy **both** decoders whatever the source says:
llvm-objdump renders 32-bit immediates with the high bit set as negative hex
(`#-0x2d800000` where GNU prints `#0xd2800000`, 20 lines) and prints `svc`'s
