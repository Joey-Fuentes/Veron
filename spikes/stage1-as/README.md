...
- **Label pool** — 88 single-char slots: `A-Za-z0-9` plus punctuation
  `_$@?!%^&~|=<>+` and `()*,-./;[]{}`. stage0-as accepts any byte as a label (its
  symtab is indexed by the raw character), so the pool can grow to serve larger
  stage-2/3 sources; the cap is the symtab size (128 entries), not the alphabet.
  Punctuation slots are only reached once a program has >62 distinct labels, so
  smaller programs resolve byte-identically to before each expansion.
...
- Up to **88** distinct labels per program (single-char pool; expandable toward
...
88-slot pool is exercised end-to-end (programs with 63–88 distinct labels
