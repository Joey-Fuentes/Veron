...
class GSym:
    __slots__ = ('is_char', 'is_array', 'size', 'init')
    def __init__(self, is_char, is_array, size, init):
    ...
    while i < len(toks):
        ...
        if toks[i] == ('punct','('):                         # ---- function ----
            ...
            while toks[i] != ('punct',')'):
                ...
                if toks[i] == ('op','*'): i += 1
                if toks[i][0] == 'id': params.append((toks[i][1], pc)); i += 1
                if toks[i] == ('punct',','): i += 1
            i += 1                                           # ')'
            depth = 0; j = i
            while j < len(toks):
            ...
            if toks[i] == ('punct','['):
                ...
                GLOBALS[name] = GSym(ischar, True, sz, None); i += 4      # [ N ] ;
            elif toks[i] == ('punct','='):
                GLOBALS[name] = GSym(ischar, False, 8, int(toks[i+1][1])); i += 3  # = num ;
            else:
                GLOBALS[name] = GSym(ischar, False, 8, None); i += 1      # ;
            gorder.append(name)
...
def compile_expr(toks, pos, frame, funcs, em, stops):
    ...
    def apply(o):
        if o in ('u-', 'u!', 'u~'):             # unary: pop one, push one
            ...
            em.i("str x0 x9"); em.i("add x9 x9 8"); return
