...
def compile_expr(toks, pos, frame, funcs, em, stops):
    ...
    def emit_index_addr(name):                  # index already on stack -> elem addr in x1
        off = frame.offset_of(name)
        em.i("sub x9 x9 8"); em.i("ldr x2 x9")              # pop index
        em.i("mov x3 3"); em.i("lsl x2 x2 x3")             # index * 8
    ...
    def push_subscript(name):                   # a[i] rvalue (index already pushed)
        emit_index_addr(name)
        em.i("ldr x0 x1"); em.i("str x0 x9"); em.i("add x9 x9 8")
    ...
    def push_deref(name):                       # *name -> load word through the pointer
        off = frame.offset_of(name)
        em.i(f"add x1 x10 {off:04d}"); em.i("ldr x1 x1"); em.i("ldr x0 x1")
...
def compile_body(em, body, frame, funcs):
    ...
    def emit_subscript_store(name):             # stack: [index, value] -> mem[base+i*8] = value
        off = frame.offset_of(name)
        em.i("sub x9 x9 8"); em.i("ldr x0 x9")             # pop value
        em.i("sub x9 x9 8"); em.i("ldr x2 x9")             # pop index
        em.i("mov x3 3"); em.i("lsl x2 x2 x3")
        em.i(f"add x1 x10 {off:04d}")
        if not frame.is_array(name): em.i("ldr x1 x1")
        em.i("add x1 x1 x2"); em.i("str x0 x1")
    while p < len(body):
        t = body[p]
        if t == ('punct', '}'):
            blk = blocks.pop()
            ...
            name = body[q][1]; q += 1
            if body[q] == ('punct', '['):                  # int NAME[ N ] — array
                n = int(body[q+1][1]); frame.declare(name, kind='array', size=n*8)
                p = q + 4                                  # skip  [ N ] ;
            else:
                frame.declare(name)
                if body[q] == ('punct', '='):
            ...
            em.i("sub x9 x9 8"); em.i("ldr x0 x9"); em.i("str x0 x1")
            p += 1
        elif t[0] == 'id':
    ...
    while i < len(body):
        if body[i] == ('kw', 'int'):
            ...
            if j + 1 < len(body) and body[j+1] == ('punct', '['):
                slots += int(body[j+2][1])                  # int NAME[N] -> N slots
            else:
                slots += 1
            i = j + 1
        else:
