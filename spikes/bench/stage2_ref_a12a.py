...
# Builtin library functions the compiler provides as a minimal runtime, lowering
# to `brk`/`svc` (A10, m51). They are a FALLBACK libc: a call to one emits a direct
# `bl` to a compiler-emitted `:name` routine, and the routine is appended to the
# program ONLY IF the builtin is actually used AND the user did not define their own
# function of that name. This mirrors the M2libc contract exactly:
#   calloc(count,size) -> a zeroed block of count*size bytes (a bump allocator over
#                         `brk`, like M2libc's `_malloc_brk`: lazy brk(0) init, grow
#                         the OS break per allocation, never recycle);
#   free(ptr)          -> a no-op for a batch compiler (M2-Planet leaks by design).
# A program that brings its own calloc/free (e.g. real M2libc, once for/switch land)
# overrides the builtin transparently.
#
# I/O primitives (A12, m53) complete the floor: a compiler reads source bytes and
# writes assembly text, so the same fallback-libc mechanism provides thin syscall
# wrappers. Each pops its args off the value stack (last arg on top), loads the
# aarch64 syscall registers, `svc`s, and pushes the return value:
#   open(name,flag,mode) -> openat(AT_FDCWD=-100, name, flag, mode)   [__NR_openat 56]
#                           (aarch64 has no bare `open`; M2libc's `_open` is openat)
#   read(fd,buf,count)   -> read                                      [__NR_read   63]
#   write(fd,buf,count)  -> write                                     [__NR_write  64]
#   close(fd)            -> close                                     [__NR_close  57]
#   exit(code)           -> exit_group-less _exit; never returns      [__NR_exit   93]
# These match M2libc/aarch64/linux/{unistd,fcntl}.c exactly, so real M2libc (which
# defines its own read/write/_open/close/_exit) transparently overrides them.
BUILTINS = {'calloc', 'free', 'open', 'read', 'write', 'close', 'exit'}
...
class Sym:
    __slots__ = ('name', 'kind', 'size', 'is_char', 'is_ptr', 'off', 'stag', 'is_fnptr')
    def __init__(self, name, kind, size, is_char, off, is_ptr=False, stag=None, is_fnptr=False):
        ...
        self.is_fnptr = is_fnptr               # a pointer-to-function (word holding a code addr)

class Frame:
    def __init__(self):
        self.by_name = {}
        self.next_off = 0
    def declare(self, name, kind='local', size=8, is_char=False, is_ptr=False, stag=None, is_fnptr=False):
        s = Sym(name, kind, size, is_char, self.next_off, is_ptr, stag, is_fnptr)
        self.by_name[name] = s
        self.next_off += size
        return s
...
class GSym:
    __slots__ = ('is_char', 'is_array', 'size', 'init', 'is_ptr', 'stag', 'is_fnptr')
    def __init__(self, is_char, is_array, size, init, is_ptr=False, stag=None, is_fnptr=False):
...
def _is_fnptr(frame, name):                    # pointer-to-function? (frame first, then globals)
    if name in frame.by_name: return frame.by_name[name].is_fnptr
    return GLOBALS[name].is_fnptr if name in GLOBALS else False
...
def _ptype_var(frame, name):                   # ptype produced by reading a variable
    if _is_fnptr(frame, name): return 0                         # code address: never scaled
...
def _is_fnptr_declarator(toks, i):
    # A function-pointer declarator begins '( *' :  ( * name ) ( params )
    return (i + 1 < len(toks) and toks[i] == ('punct', '(') and toks[i+1] == ('op', '*'))

def _fnptr_declarator(toks, i):
    # toks[i]=='(' , toks[i+1]=='*' : parse  ( * name ) ( param-list )
    # A function pointer is one machine word (a code address), so only the NAME
    # matters for codegen; the param-list is skipped. Returns (name, index_after).
    name = toks[i+2][1]
    j = i + 3
    j += 1                                     # ')' after the name
    j += 1                                     # '(' opening the param list
    depth = 1
    while j < len(toks) and depth:             # skip to the matching ')'
        if   toks[j] == ('punct', '('): depth += 1
        elif toks[j] == ('punct', ')'): depth -= 1
        j += 1
    return name, j                             # j is just past the closing ')'
...
def parse_program(toks):
    ...
    while i < len(toks):
        ...
        if _is_fnptr_declarator(toks, i):                    # ---- fnptr global: T (*f)(...) ; ----
            name, i = _fnptr_declarator(toks, i)
            GLOBALS[name] = GSym(False, False, 8, None, is_ptr=True, is_fnptr=True)
            gorder.append(name); i += 1                       # skip ';'
            continue
        isptr = False
        ...
        if toks[i] == ('punct','('):                         # ---- function ----
            ...
            while toks[i] != ('punct',')'):
                ...
                if _is_fnptr_declarator(toks, i):            # fnptr parameter: T (*f)(...)
                    pn, i = _fnptr_declarator(toks, i)
                    params.append((pn, False, True, None, True))
                else:
                    if toks[i] == ('op','*'): pp = True; i += 1
                    if toks[i][0] == 'id': params.append((toks[i][1], pc, pp, ps, False)); i += 1
                if toks[i] == ('punct',','): i += 1
            i += 1                                           # ')'
            depth = 0; j = i
            while j < len(toks):
...
def parse_functions(toks):
    ...
    while i < len(toks):
        if toks[i] != ('kw', 'int'):
            i += 1; continue
        # distinguish a function definition `int [*] name (` from a global decl to skip.
        # (The oracle does not model file-scope globals; those are ladder-verified.)
        j = i + 1
        if _is_fnptr_declarator(toks, j):                 # fnptr global -> skip to ';'
            while i < len(toks) and toks[i] != ('punct', ';'): i += 1
            i += 1; continue
        if toks[j] == ('op', '*'): j += 1
        if not (j + 1 < len(toks) and toks[j][0] == 'id' and toks[j+1] == ('punct', '(')):
            while i < len(toks) and toks[i] != ('punct', ';'): i += 1   # global var -> skip
            i += 1; continue
        name = toks[j][1]; i = j + 1
        ...
        while toks[i] != ('punct', ')'):
            ...
            if _is_fnptr_declarator(toks, i):                 # fnptr parameter
                pn, i = _fnptr_declarator(toks, i)
                params.append((pn, False, True))
            else:
    ...
    while pos < len(toks):
        ...
        if k == 'num':
            ...
            if pos + 1 < len(toks) and toks[pos+1] == ('punct', '('):
                ...
                push_member(v, chain)
            elif v in funcs:                            # bare function name -> decay to &entry
                em.i(f"adr x0 {v}"); em.i("str x0 x9"); em.i("add x9 x9 8"); tpush(0); pos += 1
            else:
                push_var(v); pos += 1
            expect_operand = False
        elif k == 'punct' and v == '(':
...
def compile_call(toks, pos, frame, funcs, em):
    ...
    pos += 1                                   # ')'
    if fname in funcs:
        em.i(f"bl {fname}")                    # direct call: args on the value stack
    elif fname in BUILTINS:                     # runtime builtin (calloc/free): direct call
        em.i(f"bl {fname}")                    #   to a compiler-emitted routine (emitted once,
        em.builtins_used.add(fname)           #   at program end, if not user-overridden)
    else:                                      # call THROUGH a function-pointer variable:
        _emit_addr(em, frame, "x1", fname)     #   x1 = &fnptr var
        em.i("ldr x16 x1")                     #   x16 = the code address it holds
        em.i("blr x16")                        #   branch-with-link through the register
    return pos
...
def compile_body(em, body, frame, funcs):
    ...
    while p < len(body):
        ...
        if t == ('punct', '}'):
            ...
            ischar = (t == ('kw', 'char'))
            q = p + 1; isptr = False
            if _is_fnptr_declarator(body, q):              # int (*f)(...) [= expr] ;
                name, q = _fnptr_declarator(body, q)
                frame.declare(name, is_ptr=True, is_fnptr=True)
                if body[q] == ('punct', '='):
...
def count_local_slots(body):
    ...
    while i < len(body):
        if body[i] == ('kw', 'struct'):
            ...
            j = i + 1
            if _is_fnptr_declarator(body, j):               # int (*f)(...) -> one word
                _, j = _fnptr_declarator(body, j)
                slots += 1; i = j; continue
...
def compile_function(em, fname, params, body, funcs):
    frame = Frame()
    for pn, pc, pp, ps, pf in params:
        frame.declare(pn, kind='param', is_char=pc, is_ptr=pp, stag=ps, is_fnptr=pf)
...
# ---------------------------------------------------------------------------
# runtime builtins (A10, m51): a minimal heap over `brk`, emitted into the
# compiled program only when used. Called with the ordinary value-stack calling
# convention (args pushed L->R, single result pushed). Both are leaf routines —
# they only issue `svc`, never `bl` — so x30 stays live and they `ret` directly.
# Only x0..x8 are touched; x9/x10/x11 (value stack / frame / frame-top) are
...
def emit_free(em):
    # no-op for a batch compiler: pop the pointer, return 0 (the callee always
    # leaves a result; a `free(x);` statement discards it, like any void-ish call).
    em.lbl("free")
    for s in ("sub x9 x9 8", "ldr x0 x9",           # pop ptr (ignored)
              "mov x0 0", "str x0 x9", "add x9 x9 8",  # push 0
              "ret"):
        em.i(s)

# --- I/O syscall wrappers (A12, m53) ---------------------------------------
# Each is a leaf routine: pop args off the value stack (the LAST argument is on
# top, so pop in reverse), load the aarch64 syscall registers x0..x3, `svc`, then
# push x0 (the kernel's return value). x9/x10/x11 are preserved; only x0..x3,x8
# are touched. `svc` is the only control-transfer, so x30 stays live and they ret.

def emit_open(em):
    # open(name, flag, mode) -> openat(AT_FDCWD, name, flag, mode). aarch64 has no
    # bare open syscall; AT_FDCWD (-100) is built as 0-100 since mov is a 16-bit MOVZ.
    em.lbl("open")
    for s in ("sub x9 x9 8", "ldr x3 x9",           # mode  (top)
              "sub x9 x9 8", "ldr x2 x9",           # flag
              "sub x9 x9 8", "ldr x1 x9",           # name  (pointer)
              "mov x0 0", "sub x0 x0 100",          # x0 = AT_FDCWD (-100)
              "mov x8 56", "svc",                    # __NR_openat -> x0 = fd (or -errno)
              "str x0 x9", "add x9 x9 8", "ret"):
        em.i(s)

def emit_read(em):
    # read(fd, buf, count) -> __NR_read (63)
    em.lbl("read")
    for s in ("sub x9 x9 8", "ldr x2 x9",           # count (top)
              "sub x9 x9 8", "ldr x1 x9",           # buf
              "sub x9 x9 8", "ldr x0 x9",           # fd
              "mov x8 63", "svc",
              "str x0 x9", "add x9 x9 8", "ret"):
        em.i(s)

def emit_write(em):
    # write(fd, buf, count) -> __NR_write (64)
    em.lbl("write")
    for s in ("sub x9 x9 8", "ldr x2 x9",           # count (top)
              "sub x9 x9 8", "ldr x1 x9",           # buf
              "sub x9 x9 8", "ldr x0 x9",           # fd
...
def emit_close(em):
    # close(fd) -> __NR_close (57)
    em.lbl("close")
    for s in ("sub x9 x9 8", "ldr x0 x9",           # fd
              "mov x8 57", "svc",
              "str x0 x9", "add x9 x9 8", "ret"):
        em.i(s)

def emit_exit(em):
    # exit(code) -> __NR_exit (93). Never returns, so no push/ret.
    em.lbl("exit")
    for s in ("sub x9 x9 8", "ldr x0 x9",           # code
              "mov x8 93", "svc"):
        em.i(s)

def emit_runtime(em, funcs):
    # a used builtin is emitted unless the user defined their own function of that
    # name (then their definition is used and the builtin steps aside — no clash).
    if 'calloc' in em.builtins_used and 'calloc' not in funcs: emit_calloc(em)
    if 'free'   in em.builtins_used and 'free'   not in funcs: emit_free(em)
    if 'open'   in em.builtins_used and 'open'   not in funcs: emit_open(em)
    if 'read'   in em.builtins_used and 'read'   not in funcs: emit_read(em)
    if 'write'  in em.builtins_used and 'write'  not in funcs: emit_write(em)
    if 'close'  in em.builtins_used and 'close'  not in funcs: emit_close(em)
    if 'exit'   in em.builtins_used and 'exit'   not in funcs: emit_exit(em)
...
def compile_program(src):
    ...
    emit_runtime(em, funcs)                     # append calloc/free iff used & not overridden
    return em.text()
...
def evaluate(src):
    ...
    def ev_expr(t, pos, env, stops):
        ...
        while pos < len(t):
            ...
            if k == 'num':
                ...
                if pos + 1 < len(t) and t[pos+1] == ('punct', '('):
                    ...
                    if v in funcs and v not in env:          # function name decays to itself
                        out.append(v)
                    else:
                        val = env.get(v, 0)
                        out.append((val & M) if isinstance(val, int) else val)
                    pos += 1
                want = False
            elif k == 'punct' and v == '(':
        ...
        while ops:
            if ops[-1] == '(':
                ...
                popop()
        r = out[-1] if out else 0
        return ((r & M) if isinstance(r, int) else r), pos
    ...
    def ev_call(t, pos, env):
        ...
        pos += 1
        target = fname
        if fname in env and isinstance(env[fname], str):   # call THROUGH a fnptr variable
            target = env[fname]
        return call(target, args), pos
    ...
    def run_body(body, env):
        ...
        while i < len(body):
            ...
            if t == ('kw', 'int'):
                j = i + 1
                if _is_fnptr_declarator(body, j):            # int (*f)(...) [= expr] ;
                    name, j = _fnptr_declarator(body, j)
                    if body[j] == ('punct', '='):
                        val, i = ev_expr(body, j+1, env, stops=(';',)); env[name] = val; i += 1
                    else:
                        env[name] = 0; i = j + 1
                    continue
                name = body[i+1][1]; i += 3
                ...
                if i + 1 < len(body) and body[i+1] == ('punct', '('):
                    _, i = ev_call(body, i, env)             # bare call statement f(...);
                    if i < len(body) and body[i] == ('punct', ';'): i += 1
                else:
                    name = t[1]; i += 2
                    val, i = ev_expr(body, i, env, stops=(';',)); env[name] = val; i += 1
            else:
    ...
    def call(name, args):
        ...
        try:
            ...
            return (r.v & M) if isinstance(r.v, int) else r.v
        return 0
