...
KEYWORDS = {'int', 'char', 'return', 'if', 'while', 'else'}
TWO_OPS  = {'<=', '>=', '==', '!=', '<<', '>>'}
...
# ---------------------------------------------------------------------------
class Sym:
    __slots__ = ('name', 'kind', 'size', 'is_char', 'is_ptr', 'off')
    def __init__(self, name, kind, size, is_char, off, is_ptr=False):
...
class Frame:
    def __init__(self):
        self.by_name = {}
        self.next_off = 0
    def declare(self, name, kind='local', size=8, is_char=False, is_ptr=False):
        s = Sym(name, kind, size, is_char, self.next_off, is_ptr)
        self.by_name[name] = s
        self.next_off += size
        return s
...
class GSym:
    __slots__ = ('is_char', 'is_array', 'size', 'init', 'is_ptr')
    def __init__(self, is_char, is_array, size, init, is_ptr=False):
...
def parse_program(toks):
    # file scope is a mix of function definitions and global declarations. Both
    # start with a type (int/char) and optional '*'; a '(' after the name marks a
    # function, otherwise it's a global (scalar, '= const', or '[N]' array).
    funcs = {}; order = []; gorder = []
    i = 0
    while i < len(toks):
        if toks[i] not in (('kw','int'), ('kw','char')):
            i += 1; continue
        ischar = toks[i] == ('kw','char'); i += 1
        isptr = False
        ...
        if toks[i] == ('punct','('):                         # ---- function ----
            ...
            while toks[i] != ('punct',')'):
                ...
                if toks[i][0] == 'id': params.append((toks[i][1], pc, pp)); i += 1
                if toks[i] == ('punct',','): i += 1
            i += 1                                           # ')'
            depth = 0; j = i
            while j < len(toks):
            ...
            if toks[i] == ('punct','['):
                ...
                GLOBALS[name] = GSym(ischar, True, sz, None, is_ptr=isptr); i += 4      # [ N ] ;
            elif toks[i] == ('punct','='):
                GLOBALS[name] = GSym(ischar, False, 8, int(toks[i+1][1]), is_ptr=isptr); i += 3  # = num ;
            else:
                GLOBALS[name] = GSym(ischar, False, 8, None, is_ptr=isptr); i += 1      # ;
            gorder.append(name)
...
def compile_function(em, fname, params, body, funcs):
    frame = Frame()
    for pn, pc, pp in params:
        frame.declare(pn, kind='param', is_char=pc, is_ptr=pp)
