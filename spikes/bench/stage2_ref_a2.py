...
Runtime model of the EMITTED program (three regions in one brk block):
  ...
  x10 = current frame base (params+locals at x10+off, declaration order, word slots)
...
class Frame:
    def __init__(self):
        self.by_name = {}
        self.next_off = 0
    def declare(self, name, kind='local', size=4):
...
def compile_expr(toks, pos, frame, funcs, em, stops):
    ...
    def push_num(nstr):
        em.i(f"mov x0 {nstr}"); em.i("str w0 x9"); em.i("add x9 x9 4")

    def push_var(name):
        off = frame.offset_of(name)
        em.i(f"add x1 x10 {off:03d}"); em.i("ldr w0 x1"); em.i("str w0 x9"); em.i("add x9 x9 4")

    def apply(o):
        em.i("sub x9 x9 4"); em.i("ldr w0 x9"); em.i("sub x9 x9 4"); em.i("ldr w1 x9")
...
def compile_body(em, body, frame, funcs):
    ...
    def store_to(name):
        off = frame.offset_of(name)
        em.i("sub x9 x9 4"); em.i("ldr w0 x9"); em.i(f"add x1 x10 {off:03d}"); em.i("str w0 x1")
    while p < len(body):
        t = body[p]
        if t == ('punct', '}'):
            blk = blocks.pop()
            ...
            p = compile_expr(body, p+1, frame, funcs, em, stops=('{',))
            em.i("sub x9 x9 4"); em.i("ldr w0 x9"); em.i("cmp x0 0")
...
def compile_function(em, fname, params, body, funcs):
    ...
    for idx in reversed(range(len(params))):   # pop args -> param slots (reverse)
        em.i("sub x9 x9 4"); em.i("ldr w0 x9")
        em.i(f"add x1 x10 {idx*4:03d}"); em.i("str w0 x1")
    compile_body(em, body, frame, funcs)
    em.i("mov x0 0"); em.i("str w0 x9"); em.i("add x9 x9 4")   # fallthrough: return 0
    emit_epilogue(em)
...
def compile_program(src):
    ...
    for s in ("mov x0 0", "mov x8 214", "svc",
              ...
              "mov x8 214", "svc",
              "bl main",
              "sub x9 x9 4", "ldr w0 x9", "mov x8 93", "svc"):
        em.i(s)
    for name in order:
