...
def cmd_mirrors(a):
    """Find and VERIFY alternate routes for every pinned artifact.

    A package is not properly pinned until there is more than one place to get
    it. Until now every recipe had exactly one working route plus a template
    pointing at a mirror nothing had been uploaded to -- which is what THIN
    meant, and which cost two runs in a row: git timed out at kernel.org, then
    freetype's SourceForge handshake timed out. Different package each time,
    same cause.

    Candidates come from two places: mechanical rewrites of known host
    families, and the URLs Arch and Alpine actually fetch from. Every one is
    downloaded and hashed before it is recorded, because a route that serves
    different bytes is worse than no route.
    """
    import tomllib
    rows, checked, found = [], 0, 0
    pkgdir = a.packages
    names = sorted(os.listdir(pkgdir))
    for pkg in names:
        rp = os.path.join(pkgdir, pkg, "recipe.toml")
        ...
        if not cands:
            print("   no candidate routes -- this pin has upstream only")
            continue

        for c in dict.fromkeys(cands):
            checked += 1
            blob = try_get(c, timeout=45, quiet=True)
            if blob is None:
                print(f"   unreachable  {c}")
                continue
            ...
            if got == sha:
                ...
                # NOT A MIRROR. Same name, different bytes -- a repackaged
                # tarball, a different release, or a compromised host. Either
                # way it must not go in the table.
                print(f"   WRONG BYTES  {c}")
                print(f"                got {got[:16]}, want {sha[:16]}")

    print(f"\n  === {found} verified routes across {checked} candidates ===")
    if not a.out:
        return 0

    # MERGE INTO THE CANONICAL TABLE, DO NOT WRITE BESIDE IT.
    #
    # The first version wrote these rows to a separate file, and nothing read
    # it. So freetype had four verified SourceForge routes recorded in
    # MIRRORS-verified.tsv while `mirror fetch` -- which reads MIRRORS.tsv --
    # still had only the redirector, and the very next run died on its TLS
    # handshake. A discovered route that the fetcher cannot see is not a
    # route.
    existing, header = [], []
    ...
    merged = list(dict.fromkeys(existing + [tuple(r) for r in rows]))
    with open(a.out, "w") as f:
        f.write("\n".join(header) + "\n")
        for row in sorted(merged, key=lambda r: (r[1], r[2], r[3])):
            f.write("\t".join(row) + "\n")
    added = len(merged) - len(existing)
    print(f"  {a.out}: {len(existing)} rows in, {added} added, "
          f"{len(merged)} total")
    return 0
