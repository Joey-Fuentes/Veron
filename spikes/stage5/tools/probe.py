...
def cmd_mirrors(a):
    ...
    if a.out:
        with open(a.out, "w") as f:
            ...
            f.write("# Generated by `probe mirrors`. Every row was downloaded\n")
            f.write("# and its sha256 compared against the pin before being\n")
            f.write("# written. Regenerate, never hand-merge.\n")
            for row in rows:
                f.write("\t".join(row) + "\n")
        print(f"  wrote {a.out}")
    return 0
