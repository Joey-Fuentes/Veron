...
if [ "$R1" = ok ]; then
  ...
  for s in src/fenv/x86_64/fenv.s src/ldso/x86_64/tlsdesc.s \
           src/process/x86_64/vfork.s src/setjmp/x86_64/longjmp.s \
           src/signal/x86_64/restore.s src/thread/x86_64/__set_thread_area.s \
           src/thread/x86_64/__unmapself.s src/thread/x86_64/clone.s \
           src/thread/x86_64/syscall_cp.s; do
    [ -f "$s" ] || continue
  ...
  sed -f tools/mkalltypes.sed arch/x86_64/bits/alltypes.h.in \
      include/alltypes.h.in > obj/include/bits/alltypes.h 2>/dev/null
  cp arch/x86_64/bits/syscall.h.in obj/include/bits/syscall.h 2>/dev/null
  sed -n 's/__NR_/SYS_/p' arch/x86_64/bits/syscall.h.in \
  ...
  #     musl/arch/x86_64/bits/float.h    LDBL_MANT_DIG 113
  #
  ...
  if [ "$LD_IS_DOUBLE" = yes ] && [ -f arch/x86_64/bits/float.h ]; then
    cat > arch/x86_64/bits/float.h <<'FLOATH'
...
FLOATH
  ...
  INC="-Iarch/x86_64 -Iarch/generic -Iobj/src/internal -Isrc/include -Isrc/internal -Iobj/include -Iinclude"
  ...
  #
  # The last run skipped src/string/memset.c because src/string/x86_64/memset.S
  ...
  for f in src/*/*.c src/malloc/mallocng/*.c ldso/*.c compat/time32/*.c; do
    ...
    if [ -f "$d/x86_64/$b.c" ] || [ -f "$d/x86_64/$b.s" ] || [ -f "$d/x86_64/$b.S" ]; then
      continue
    fi
    echo "$f" >> /work/srclist
  done
  for f in src/*/x86_64/*.c src/*/x86_64/*.s src/*/x86_64/*.S \
           crt/*.c crt/x86_64/*.c crt/x86_64/*.s crt/x86_64/*.S; do
  ...
  # something like /usr/lib/x86_64-linux-gnu, not /usr/lib.
  #
  ...
  for c in crt1 crti crtn Scrt1 rcrt1; do
    for src in "obj/crt/x86_64/$c.o" "obj/crt/$c.o"; do
      [ -f "$src" ] || continue
      for d in $crtdirs; do
  ...
  cp -a arch/x86_64/bits "$SYS/include/" 2>/dev/null || true
...
#
#     /work/bld/gcc/as: exec: .../x86_64-unknown-linux-gnu/bin/as:
#                             Permission denied
#
...
# x86_64-toolchain-linux-musl differs from x86_64-unknown-linux-gnu, and it is
...
# configures gcc --build/--host/--target=x86_64-veron-linux-gnu. The paths at
# $PFX/x86_64-unknown-linux-gnu/ below belong to phase A's own gcc 4.7 and
# are thrown away with the box.
BUILDTRIP=x86_64-unknown-linux-gnu
HOSTTRIP=x86_64-toolchain-linux-musl
...
ARSHIM
  ...
  if timeout 3000 make -j"$NP" MAKEINFO=true AR="$AR" RANLIB="$RANLIB" > build.log 2>&1 \
    ...
    #
    #     -B$PFX/x86_64-unknown-linux-gnu/bin/  -isystem .../include
    #     -B$PFX/x86_64-unknown-linux-gnu/lib/  -isystem .../sys-include
    #
    # and it died with
    #
    #     /work/bld/gcc/as: exec: /work/prefix/x86_64-unknown-linux-gnu/bin/as:
    #                             Permission denied
    #
    ...
    _TD="$PFX/x86_64-unknown-linux-gnu"
    ...
    for _p in /work/out /work/out2 /work/out10; do
      mkdir -p "$_p/x86_64-unknown-linux-gnu/bin" "$_p/x86_64-unknown-linux-gnu/lib"
      for _t in as ld ar ranlib nm objcopy objdump strip readelf strings; do
        [ -x "$PFX/bin/$_t" ] || continue
        rm -f "$_p/x86_64-unknown-linux-gnu/bin/$_t"
        ln -s "$PFX/bin/$_t" "$_p/x86_64-unknown-linux-gnu/bin/$_t"
      done
      rm -rf "$_p/x86_64-unknown-linux-gnu/sys-include"
      ln -s /usr/include "$_p/x86_64-unknown-linux-gnu/sys-include"
    done
...
if [ "$R5" = ok ]; then
  ...
  if [ "$R6" != FAIL ]; then
    ...
    # NO BACKPORT ON THIS TARGET, AND THE COUNTS SAY WHY.
    #
    # The aarch64 arm asserts `expect 0` above and then transplants 4.8.5's
    # backend in, because gcc 4.7 has no aarch64 port at all -- it arrived in
    # 4.8. x86_64 is 4.7.4's oldest and best-supported target, so the same
    # file is checked for the opposite answer and nothing is applied.
    #
    # THE TWO COUNTS TOGETHER ARE THE STATEMENT. One says this target needs
    # no transplant; the other says why the other arm cannot skip it.
    _nx=$(grep -c x86_64 "$g47/gcc/config.gcc" 2>/dev/null || true)
    _na=$(grep -c aarch64 "$g47/gcc/config.gcc" 2>/dev/null || true)
    say "    stock 4.7.4: $_nx x86_64 mentions in config.gcc  (expect many)"
    say "    stock 4.7.4: $_na aarch64 mentions in config.gcc (expect 0 -- why that arm backports)"
    if [ "${_nx:-0}" -lt 1 ]; then
      say "    4.7.4 HAS NO x86_64 BACKEND -- this target is not what it claims"
      R6=FAIL
    fi
  fi
  ...
  # libgcc's configure runs the freshly built xgcc with
  #
  #     -B$PFX/x86_64-unknown-linux-gnu/bin/  -B.../lib/
  ...
  # $PFX/x86_64-unknown-linux-gnu/. Printing it before the build turns a
  # "cannot compute suffix of object files" forty minutes in into one line
  # here.
  say "    --- what xgcc will be given for the target ---"
  for _d in "$PFX/x86_64-unknown-linux-gnu/bin" "$PFX/x86_64-unknown-linux-gnu/lib" \
            "$PFX/x86_64-unknown-linux-gnu/include" "$PFX/x86_64-unknown-linux-gnu/sys-include"; do
    if [ -d "$_d" ]; then
  ...
  if "$PFX/x86_64-unknown-linux-gnu/bin/as" --version >/dev/null 2>&1; then
  ...
  if [ "$R6" != FAIL ] && timeout 5400 make -j"$NP" MAKEINFO=true > build.log 2>&1; then
    ...
    #     internal compiler error: in ?, at config/x86_64/aarch64-builtins.c:944
    ...
    #                        in ?, at config/x86_64/aarch64-builtins.c:944
    #
...
# toolchain targeting x86_64-veron-linux-gnu cannot silently reach anything
# built for x86_64-unknown-linux-gnu, so a leak from the old sysroot becomes a
...
VERON_TOOLCHAIN_TGT=x86_64-toolchain-linux-gnu
S=/work/lfs
...
# in the first place; the note matters because gcc's t-x86_64-linux is what
...
#
# THE t-x86_64-linux SED IS OURS, NOT THE BOOK'S. LFS seds
# gcc/config/i386/t-linux64 for x86_64; the aarch64 file doing the same job is
# t-x86_64-linux. Without it glibc lands in /usr/lib and libstdc++ in
...
if [ "$R14" = ok ]; then
  ...
  if ! untar /in/busybox-; then
    ...
    cd "/work/src/$_bb"
    _BBMAKE="make ARCH=x86_64 CROSS_COMPILE=$VERON_TOOLCHAIN_TGT- HOSTCC=$PFX/bin/chain-cc"
...
if [ "$R14" = ok ]; then
  ...
  #
  #     checking whether x86_64-veron-linux-gnu-g++ supports C++14 ... yes
