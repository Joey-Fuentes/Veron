...
# /usr/bin/riscv64-veron-linux-gnu-gcc. The prefixed name is the SYSROOT's
...
_xp=$(command -v "${VERON_TOOLCHAIN_TGT:-riscv64-toolchain-linux-gnu}-gcc" 2>/dev/null || true)
case "$_xp" in
  /tools/*) say "    $_xp RESOLVES INTO /tools -- that is pass 1, the cross"
            ...
            _ok=no ;;
  "")       say "    no ${VERON_TOOLCHAIN_TGT:-riscv64-toolchain-linux-gnu}-gcc on PATH" ;;
  *)        say "    ${VERON_TOOLCHAIN_TGT:-riscv64-toolchain-linux-gnu}-gcc -> $_xp"
...
if [ "$B2" = ok ]; then
  if fetch "/in/binutils-$BINUTILS_LFS" binutils; then
    ...
    # configures gcc as riscv64-veron-linux-gnu, so it looks in
    # /usr/riscv64-veron-linux-gnu/bin/ -- and binutils, configured natively,
    ...
    if "$_d/configure" --prefix=/usr \
         --build=riscv64-veron-linux-gnu \
         --host=riscv64-veron-linux-gnu \
         --target=riscv64-veron-linux-gnu \
...
if [ "$B3" = ok ]; then
  if fetch "/in/gcc-$GCC15" gcc; then
    ...
    _t="$_d/gcc/config/riscv/t-linux64"
    if [ ! -f "$_t" ]; then
    ...
    # the triplet and answered riscv64-unknown-linux-gnu. Measured: 678
    ...
    # change: $VERON_TOOLCHAIN_TGT becomes riscv64-toolchain-linux-gnu, because a cross
    # compiler in /tools called riscv64-veron-linux-gnu-gcc reads as THE Veron
