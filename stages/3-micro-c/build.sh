...
do_in() {
  ...
  # tccdefs_.h IS GENERATED, AND HERE IT IS GENERATED BY US: CONFIG_TCC_PREDEFS
  # bakes include/tccdefs.h into the compiler as C strings, which upstream
  # produces by compiling conftest.c -DC2STR with the HOST compiler. The old
  # base carried a runner-gcc-built copy; the pristine pin has none, and the
  # ladder's first run on Veron stopped on exactly that (tccpp.c:3728). The
  # converter is ported to tools/c2str.py, the output is committed beside
  # config.h, and every run regenerates it and refuses a byte of drift --
  # the generator is re-proven each time rather than trusted once.
  python3 "$ROOT/tools/c2str.py" "$IN/tcc-src/include/tccdefs.h" "$IN/tccdefs_.h.gen" \
    || { echo "FAIL: tools/c2str.py could not convert include/tccdefs.h"; exit 1; }
  cmp -s "$IN/tccdefs_.h.gen" "$ROOT/stages/3-micro-c/tcc/tccdefs_.h" \
    || { echo "FAIL: regenerated tccdefs_.h differs from the committed one -- tccdefs.h or c2str.py changed; review, then commit the new file"; exit 1; }
  cp "$ROOT/stages/3-micro-c/tcc/tccdefs_.h" "$IN/tcc-src/tccdefs_.h"
  echo "  tccdefs_.h regenerated by tools/c2str.py and matches the committed file"
