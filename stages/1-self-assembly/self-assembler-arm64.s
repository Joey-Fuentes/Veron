...
// cross-check.
// ============================================================================
// Veron — Stage 1 Self-Assembly: self-assembler  (ARM64 / AArch64)
// ============================================================================
//
...
//     self-assembler: rejected: <the line>
...
//
// Every instruction below is inside the subset self-assembler itself accepts, so
...
h_orr:
    ...
    orr     w9, w9, w25, lsl #5
    orr     w9, w9, w24
    // OPTIONAL SHIFT AMOUNT. `orr Rd Rn Rm 8` is 33 of the 55 lines self-assembler
...
h_bcond:
    ...
    // branch. Nothing above self-assembler emits b.gt or b.le -- pico-c-assembler (stage 2) and
    // stage2-pico-c.s1 contain none -- which is why the ladder was unaffected.
    // self-assembler's OWN source uses them eight times, so self-hosting on the old
...
// a source line.
    .section .rodata
    .balign 8
inover:  .ascii  "self-assembler: input exceeds INBUF_SZ\n"
rejmsg:  .ascii  "self-assembler: rejected: "
rejnl:   .ascii  "\n"
