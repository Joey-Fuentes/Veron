...
- [`spikes/stage5/AMD64.md`](./spikes/stage5/AMD64.md) — **stage 5 has a second architecture, and nothing has been built on it yet.** An x86_64 arm of the stage-5 workflow, its sysroot and kernel taken from one stage-4 release; the seven recipes that name aarch64 replaced through an overlay rather than edited, so the aarch64 arm's tree and plan are untouched; and the gate that refuses to let the two copies of a recipe disagree about what they build. Four of the seven decline their x86 assembly for want of a `nasm` nobody has pinned, and that cost is declared rather than discovered later
