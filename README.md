...
- [`spikes/stage5/AMD64.md`](./spikes/stage5/AMD64.md) — **stage 5 on x86_64: 45 of 122 packages, and one finding worth the whole arm.** The recipes that name aarch64 are replaced through an overlay rather than edited, so the aarch64 tree and plan are untouched, and a gate refuses to let the two copies of a recipe disagree about what they build. The stop at package 46 was not an architecture quirk: `freetype` has declared `bzip2` declined since it was written and never told configure, which autodetected it and linked the static `libbz2.a` — accepted into a shared library on aarch64, refused on x86_64. A static archive leaves no `DT_NEEDED` entry, so the detector written to catch exactly this could not see it, and **the aarch64 image ships a `libfreetype.so` with bzip2 compiled into it under a recipe that says otherwise.** Three detectors were right and the build was wrong; the second architecture is what found it
