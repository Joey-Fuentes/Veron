...
- [`spikes/stage5/AMD64.md`](./spikes/stage5/AMD64.md) — **stage 5 on x86_64: all 122 packages build, the image reproduces byte-for-byte, and it boots.** Four runs, and every stop was a defect in the base tree that aarch64 tolerates rather than an architecture quirk. bzip2 builds a shared library and never stages it, so `freetype` gets a bzip2 it declared **off** and `libarchive`, `python` and `cmake` cannot have the one they declared **on** — and because aarch64 accepts non-PIC objects into a shared library where x86_64 refuses, the published aarch64 image carries bzip2's code *inside* three libraries with no `DT_NEEDED` entry naming it, the one field the dependency detector reads. Then `libffi`, the only package passing `--with-gcc-arch=native`, was compiled for the Xeon that built it and hit an invalid opcode under the emulator the image boots on; its own recipe named that mechanism in advance and filed it as a reproducibility risk, which it is, and also a portability one. The recipes that differ live in an overlay rather than being edited, so the aarch64 tree and plan are untouched, and a gate refuses to let the two copies of a recipe disagree about what they build
