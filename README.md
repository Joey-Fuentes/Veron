...
micro-c now compiles **tcc's whole source, driver included**, and `stage3-hermetic-arm64` reports **`stage 3 end to end: yes`** — a 1.58 MB tcc built from the seed on native ARM64 inside the sealed box, answering `tcc version 0.9.28rc (AArch64 Linux)` from tcc's own driver, with all twelve end-to-end programs compiling and running. `-E`, `-c` and multiple input files work because they are tcc's own code. Read it narrowly: all twelve are small enough to stay below the `realloc` threshold where larger inputs still fail. The last fault closed was that micro-c never truncated a narrowing cast, so tcc's hand-rolled 32-bit sign extension at `arm64-gen.c:494` ran on a value that had never been truncated and every local offset came out 2^32 too small — which is why taking the address of a local aggregate segfaulted while scalars and globals worked. What remains is one fault, diagnosed and not yet fixed: a pending `*` lands on a function's **argument** rather than its result, so `*(uint8_t*)section_ptr_add(s,1) = data` — how tcc writes every byte of `.eh_frame` — stores through a wrongly computed address and corrupts the heap. It surfaces much later as `realloc: pointer was never returned by malloc`. See `spikes/stage3/MICRO-C.md`.
