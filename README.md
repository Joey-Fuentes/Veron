...
micro-c now compiles **tcc's whole source, driver included** — `mc-tcc --version` answers `tcc version 0.9.28rc (AArch64 Linux)`, and `-E`, `-c` and multiple input files work because they are tcc's own code. It compiles and runs all twelve of the end-to-end programs — though all twelve are small enough to stay below the `realloc` threshold where larger inputs now fail, so that is a narrower result than it sounds. The last fault closed was that micro-c never truncated a narrowing cast, so tcc's hand-rolled 32-bit sign extension at `arm64-gen.c:494` ran on a value that had never been truncated and every local offset came out 2^32 too small — which is why taking the address of a local aggregate segfaulted while scalars and globals worked. Swept against tcc's own 129-test `tests2` suite with a gcc-built control, 46 of 57 real failures are a single fault: `realloc` is handed an address that was never a block. See `spikes/stage3/MICRO-C.md`.
